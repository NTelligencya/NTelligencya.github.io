...
- [x] Built as "Can you tell if it's AI? Watermarking explained" at /presentations/ai-watermarking-explained/ (interactive-walkthrough-deck pattern, shipped 21 August 2026; companion course page at /cdu-teaching-staff/ai-watermarking-and-provenance.html). Front-page workshops card since 21 August 2026 (a duplicate card from 20 August was removed 30 August 2026).
- DONE 30 August 2026: Module 06 "Try it yourself: the watermark playground" added at the end of the deck (5 parts, 29 parts total). Three-panel playground modelled on the public LLM Watermarking Playground demo but with no in-browser model: a stand-in template model with two passages (a choice-rich Darwin wet-season description, and the CDU early history restating the Wikipedia facts as the low-entropy case), modes None / Hard / Soft with gamma, delta, context width h and key, token colouring by green and red list, the paper's Equation 3 z-score and one-sided p-value with the z = 4 threshold, a wrong-key demonstration, a "count repeated pairs once" option, and a Paraphrase button that re-chooses a quarter of the choice points unmarked. SD's CDU Wikipedia history paragraph is the pre-loaded human control text. Measured in testing: human text z about -1.5; wet-season Hard z about 7.7 (always detected), Soft delta 2 about 5.1, delta 1 about 3.0; CDU history Hard about 2.1 (weak signal, never detected, by design); wrong key about 0. Deep links: #playground or #m6 open the module directly; digit key 6; module 5 closer links to it. Readability gate for the new module: Grade 4.0, all sentences under 35 words. Cards on index.html, presentations/index.html and the course page updated to 6 modules and mention the playground. Search index not regenerated (no new pages).
- DONE 16 September 2026: course page /cdu-teaching-staff/ai-watermarking-and-provenance.html rebuilt as the presenter's scroll-through page, ending at the playground. Folded in the deck's substantive content (Article 50(2) text with the carve-out highlighted, rollout timeline, Taboo analogy, worked z-score number, code low-entropy example, humaniser cycle, Pangram/Jabarian/Narayanan detection-at-scale list) and reuses the deck's three exhibits by absolute path. New: a plain-language unpacking of "signed metadata" (metadata = label, manifest = record, signed = seal; medicine-bottle tamper band analogy) with an SVG "what happens to the seal" figure, SD's screenshots of contentcredentials.org (full page plus a crop of the penguin credential panel) and claude.com/check-files as exhibits (exhibit-*.jpg/.webp in the folder); a "who is marking what, and who can check" provider table; a marking-office callout (no one at CDU can run a text watermark check; Turnitin-style scores are stylistic detectors); the three EC links (Article 50 service desk, Code of Practice, EU icons) and the Anthropic help article, all target=_blank. Research refresh (16 Sept 2026): Anthropic disclosed on 14 Aug 2026 that Claude uses Google's SynthID-Text method (not an undisclosed scheme); marking is on supported models only (help article names Fable 5.1 and Mythos 5.1, older models being added); global because Anthropic has no durable way to scope by region; text detector API is private preview for eligible orgs incl. educational organisations; free file checker at claude.com/check-files (also /check-content). Final Code of Practice 10 June 2026; ~190 signatories by 31 July incl. Anthropic, Google, Meta, Microsoft, OpenAI, Mistral; Google SynthID partners Apple, ElevenLabs, Kakao, NVIDIA, OpenAI (24 July); EU icons page 10 Aug; Art 99(4) fines EUR 15m / 3 per cent; transition to 2 Dec 2026 for systems already on the market; only Anthropic and Google publicly confirm text marking; OpenAI text mark not confirmed. New academic source: Nemecek, Chaudhary and Ayday, arXiv 2609.09604, 9 Sept 2026 (unverifiability; code detection near chance). The "Two claims left out on purpose" drafting callout was removed from the page and the matching "Deliberately not used" sentence from the deck's Module 05 sources. Deck text edited for accuracy only (five sentences about the "undisclosed" scheme and the worldwide-default claim); interactive elements untouched. Search index not regenerated (no new pages).
- Original hook title: "Real or AI? How Watermarks and Detectors Actually Work"
...
- DONE 22 August 2026: Cognitive Uploading walkthrough deck at /presentations/cognitive-uploading/ (interactive-walkthrough-deck pattern, 4 modules, 22 parts; note the skill's "beats" are now called "parts" per SD, and the "Myth we'll unpick" opener pattern is retired in favour of a plain focus chip). Covers Gemini Notebook (renamed from NotebookLM 16 July 2026), source grounding, Steven Johnson's cognitive uploading essay (Adjacent Possible, 2 June 2026), the offloading objection (Risko and Gilbert 2016; Gerlich 2025), and Sen/Workiewicz/Puranam, Strategy Science 2026 (LLM high recall low precision vs human high precision low recall). Card added to /presentations/index.html (hero-keyboard-closeup art). Palette is the sanctioned teal set plus a steel-blue secondary (#4b85a9) per SD's request to echo workshops/caring-for-your-data. Built for the CDU teaching-staff course's Cognitive outsourcing section (Track 2, see the workshop folder's CLAUDE.md); that section page will link it when built. Readability gate passed at Grade 7.1; Playwright drive-through clean. Dropped as unverifiable: the "Ferrari in a school zone" line (not Forte's) and Johnson's claimed refusal to publish AI-written books (not in the essay). Search index NOT yet regenerated (run node tools/build-index.js --sync --dates-from-git).
- DONE 21 August 2026: Watermarking, as the /presentations/ai-watermarking-explained/ walkthrough deck (playground module added 30 August 2026; see the topic entry above).
- NEXT: Philosophical Futures (video embed + source list ready), Agentic AI (plus its /presentations/ explainer version).
