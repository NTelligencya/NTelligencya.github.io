import { TYPES, clone, key, endpoint, getNode, getPort, isHost, maskText, sameSubnet, addNode, connect, removeNode, configurePort, tracePing, makeScenario, validateImport } from './engine.mjs?v=20260914.2';
...
const lessons = {
  ...
  sandbox: { title: 'Build your own network and test an idea', text: 'Add devices, connect ports, configure addresses and choose two end devices to test. Try an incorrect mask, a missing gateway, a disabled link or duplicate addresses. Predict the effect before each test.', hints: [], question: '' }
};
...
$('export').addEventListener('click',()=>{const url=URL.createObjectURL(new Blob([JSON.stringify(pack(),null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='ntworldink-lan-network.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);notice('Network exported, including activity progress.');});
