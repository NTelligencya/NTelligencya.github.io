...
- `prompt-gallery-assets/` and `prompt-gallery-outputs/`: the 1600 x 1000 showcase screenshots for the Basic Prompts Gallery, and the model outputs they were taken from (generated 20 September 2026 by running each gallery prompt in a fresh context). The output pages carry a noindex tag and the folder is in `SKIP_DIRS` in `tools/build-index.js`, so they stay out of search and the sitemap. `most-wanted.html` states real law and penalties; recheck it against `prompt-gallery-outputs-verification.md` (kept outside the repo) periodically, since penalty amounts and the Commonwealth penalty unit value change over time.
